...
//----------------------------角度控制PD------------------------------//
float     Angle_Control_P = 80;                        
float     Angle_Control_D = 0.8;  

float     Angle_Control_Out_Max =7100.0;
 
float     g_Angle_Control_Out=0;//角度控制输出
//float     g_Speed_Control_Out=0;//角度控制输出


//----------------------------电机死区---------------------------------//
float     g_Left_Motor_Dead_Value =825;//
float     g_Right_Motor_Dead_Value = 800;
...
void read(void)
{
		USART2_SendString("left is %f\n",g_Left_Motor_Dead_Value);
	  USART2_SendString("right is %f\n",g_Right_Motor_Dead_Value);
	USART2_SendString("P is %f\n",Angle_Control_P);
	  USART2_SendString("d is %f\n",Angle_Control_D);
}

void testleft(float a)
{
	g_Left_Motor_Dead_Value +=a;
...
void Set_control_Pwm(float g_Speed_Control_Out,float g_Direction_Control_Out)
{
    ...
    Left_Motor_Voltage =g_Angle_Control_Out +g_Speed_Control_Out+ g_Direction_Control_Out;
    Right_Motor_Voltage =g_Angle_Control_Out +g_Speed_Control_Out- g_Direction_Control_Out;
   if(Left_Motor_Voltage > 0)
    {
        Left_Motor_Voltage += 1000.0;
    }
    else if(Left_Motor_Voltage < 0)
    {
        Left_Motor_Voltage -= 1000.0;
   ...
   if(Right_Motor_Voltage > 0)
    {
        Right_Motor_Voltage += 1050;
    }
    else if(Right_Motor_Voltage < 0)
    {
        Right_Motor_Voltage -= 1050.0;
    }
	/*
		if(Angle>30||Angle<-30)//跌倒停止
	{
